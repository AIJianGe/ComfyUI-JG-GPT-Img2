...
class JiangeGPTImage2OfficialStableNode:
    _SIZE_CHOICES = [
        "auto",
    ...
    DESCRIPTION = "GPT Image 2 官方稳定版 @AI剑歌"
    ...
    def _build_edits_multipart(self, prompt, images, mask, n, quality, size, background, output_format, output_compression, moderation):
        if mask is not None and not images:
            raise Exception("使用遮罩时必须提供参考图")
        image_files = []
    ...
    def _async_edits(self, base_url, prompt, images, mask, pbar, max_poll_attempts, poll_interval, webhook, n, quality, size, background, output_format, output_compression, moderation, max_retries, initial_timeout):
        data, request_files = self._build_edits_multipart(
            prompt, images, mask, n, quality, size, background, output_format, output_compression, moderation,
        )
    ...
    def generate(self, **kwargs):
        ...
        n = _si(kwargs.get("🖼️ 图片数量"), 1)
        quality = kwargs.get("🎨 画质", "auto")
        size = kwargs.get("📐 尺寸", "auto")
        background = kwargs.get("🌈 背景", "auto")
        output_format = kwargs.get("📦 输出格式", "png")
        output_compression = _si(kwargs.get("🗜️ 压缩质量"), 100)
        moderation = kwargs.get("🛡️ 审核强度", "auto")
        async_mode = bool(kwargs.get("⚡ 异步模式", True))
        webhook = kwargs.get("🔔 Webhook", "")
        max_poll_attempts = _si(kwargs.get("🔁 最大轮询次数"), 300)
        poll_interval = _si(kwargs.get("⏱️ 轮询间隔"), 5)
        max_retries = _si(kwargs.get("♻️ 最大重试次数"), 5)
        initial_timeout = _si(kwargs.get("⌛ 初始超时"), 900)
        ...
        try:
            if async_mode:
                combined, image_url, task_id, final_result = self._async_edits(
                    base_url, prompt, images, mask, pbar, max_poll_attempts, poll_interval, webhook, n, quality, size, background, output_format, output_compression, moderation, max_retries, initial_timeout,
                )
                info = self._build_info(
                    api_source, prompt, quality, size, background, output_format, output_compression, moderation, image_url, task_id, final_result, True
                )
            ...
            for item in data_array:
                ...
                if url:
                    first_url = url
                    break
            info = self._build_info(api_source, prompt, quality, size, background, output_format, output_compression, moderation, first_url, "", result, False)
...
NODE_CLASS_MAPPINGS = {
    "GPT_Image_2_官方稳定版": JiangeGPTImage2OfficialStableNode,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "GPT_Image_2_官方稳定版": "⚔️GPT_Image_2_官方稳定版",
}
