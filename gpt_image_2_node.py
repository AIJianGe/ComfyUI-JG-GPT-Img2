...
class JiangeGPTImage2Node:
    ...
    RETURN_NAMES = ("🖼️ 参考图", "🔗 图片链接", "🆔 任务ID", "📋 响应信息")
    FUNCTION = "process"
    CATEGORY = "🤖GPT-Image-2"
    DESCRIPTION = "GPT Image 2 图像生成/编辑 @AI剑歌"
    ...
    def _get_base_url(self, api_source, custom_api_url):
        ...
        if api_source == "ip" and not url:
            print("[GPT-Image-2] ⚠️ 选择了 ip 线路但未填写自定义API地址，将默认使用柏拉图")
    ...
    def _process_image_items(self, items):
        ...
        for item in data_items:
            try:
                ...
                print(f"[GPT-Image-2] 处理返回图片失败: {e}")
    ...
    def _query_task_status(self, base_url, api_key, task_id, response_info, pbar):
        try:
            ...
            print(f"[GPT-Image-2] GPT Image 2 查询状态: {status}")
    ...
    def _poll_task(self, base_url, api_key, task_id, response_info, pbar):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                result = response.json()
                status, final_data = self._extract_status_and_data(result)
                print(f"[GPT-Image-2] GPT Image 2 轮询状态: {status}")
                ...
                print(f"[GPT-Image-2] GPT Image 2 轮询异常: {e}")
        return self._blank_result({
            "status": "timeout",
            "task_id": task_id,
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if mode == "text2img":
                ...
                if seed > 0:
                    data["seed"] = str(seed)
                print(f"[GPT-Image-2] GPT Image 2 图生图提交: size={request_size}, images={len(files)}, input_fidelity={data.get('input_fidelity', 'auto')}")
                response = requests.post(
            ...
            result = response.json()
            print(f"[GPT-Image-2] GPT Image 2 提交响应: {json.dumps(result, ensure_ascii=False)[:1000]}")
            ...
            print(f"[GPT-Image-2] GPT Image 2 执行异常: {e}")
            return self._blank_result({
                "status": "failed",
...
NODE_CLASS_MAPPINGS = {
    "GPT_Image_2_异步": JiangeGPTImage2Node,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "GPT_Image_2_异步": "⚔️GPT_Image_2_异步",
}
